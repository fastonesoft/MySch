    ...
    public class UserGradeController : RoleController
    {
        ...
        public ActionResult Add()
        {
            try
            {
                ...
                var partsteps = QPartStep.GetEntitys(a => a.AccIDS == login.IDS);

                ViewBag.Edus = Combo.ToComboJsons<BllEdu>(edus, null);
                ViewBag.Years = Combo.ToComboJsons<BllYear>(years, null);
                ViewBag.PartSteps = Combo.ToComboJsons<QPartStep>(partsteps, null);
        ...
        public ActionResult Edit(string id)
        {
            try
            {
                ...
                ViewBag.PartSteps = Combo.ToComboJsons<QPartStep>(partsteps, entity.PartStepIDS);
        ...
        public ActionResult AddTokey(BllGrade entity)
        {
            try
            {
                ...
                //查询 视图数据
                var qentity = QGrade.GetEntity(a => a.ID == entity.ID);
                return Json(qentity);
            }
            catch (Exception e)
            {
        ...
        public ActionResult DataGrid(int page = 1, int rows = 100)
        {
            ...
            {
                var login = BllLogin.GetLogin(Session);
                var res = QGrade.GetDataGridPages(a => a.AccIDS == login.IDS, page, rows);
                return Json(res);
            }
            catch (Exception e)
            {
