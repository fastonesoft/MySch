...
namespace MySch.Controllers.User
{
    public class UserPartController : RoleController
    {
        ...
        public ActionResult AddToken(BllPart entity)
        {
            try
            {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditToken(BllPart entity)
        {
            try
            {
                var login = BllLogin.GetLogin(Session);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult DelToken(BllPart entity)
        {
            try
            {
                var login = BllLogin.GetLogin(Session);
        ...
        public ActionResult DataGrid(int page = 1, int rows = 100)
        {
            try
            {
                ...
                var res = BllPart.GetDataGridPages<BllPart, string>(a => a.AccIDS == login.IDS, a => a.IDS, page, rows);
                return Json(res);
            }
            catch (Exception e)
            {
